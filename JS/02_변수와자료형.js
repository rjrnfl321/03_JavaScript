...
// =(등호) : 대입 연산자
// A = B -> B를 A에 대입 (오른쪽 값을 왼쪽 변수에 대입) *항상
...
//------------------------------------------------
/* 1.var  */
/*  변수, 변수명 중복 허용(덮어쓰기), 함수 레벨 범위(scope)  */
function varCheck() {
  var menu = "콩국수";
...
/* 3. const */
/* 상수(constant,항상 같은 수), 변수명 중복 허용X, 블록{} 레벨 범위() */
...
function constCheck() {
  ...
  // 해결 방법 :  const nationaCode; -> let nationalCode;
...
/* 선수 지식
   1. 전역 변수 : 어디서든 사용 가능한 변수 (ex T머니)

   2. 지역 변수 : 특정 지역에서 사용 가능한 변수 (ex 기후동행카드)
      1) 함수 레벨 범위
          - function 함수명 (){}
            함수 정의 부분 {} 내부에서
...
function scopeTest2() {
  ...
  if (1 == 1) { // 1과 1이 같다면 if {} 수행
    const locall1 = "if 지역 변수1";

    console.log("if 내부에서 globall", globall); // 출력 O
    ...
    console.log("if 내부에서 locall1 : ", locall1); // 출력 O

    fn2 = "변경된 fn2";

    // [좁은 범위의 데이터를 넓은 범위에서 사용할수 있게 하는 방법]
    // if {} 내부(좁은 범위) 에서
...
function typeCheck() {
  ...
  // undefined : 정의되지 않은 변수 / 값 대입이 안되있는 상태 또는 존재 X 변수
  ...
  console.log("phone : ", phone, typeof name);
  ...
  console.log("numbers 배열의 값 중[0]번째 : ", numbers[0]
    , typeof numbers[0]);
  console.log("numbers 배열의 값 중[1]번째 : ", numbers[1]
    , typeof numbers[1]);
  console.log("numbers 배열의 값 중[2]번째 : ", numbers[2]
    , typeof numbers[2]);
