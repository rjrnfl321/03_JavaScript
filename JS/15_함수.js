...
/** num의 x 제곱을 반환하는 함수 */
function pow(num, x) {
  let result = 1; // 곱할 떄 영향을 주지 않는 1로 초기화
...
btn6.addEventListener/*이벤트*/("click"/*행동*/, function/*처리*/() { //익명함수
  console.log("익명 함수 실행");
  /* 변수 + 익명 함수 */

  // 변수는 함수도 저장할 수 있다 ( 자료형 : function)
  // 변수명이 익명 함수의 함수명 역할을 해준다
  // (요즘 함수 정의 방법 (function 안쓰기가 트렌드다))
...
arrows[1].addEventListener("click", () => {
  // 매개 변수 1개 -> () 생략 가능 , 매개 변수가 여러개면 문법상 오류 발생
  const testFn = num => {
    return 10 * num; // num에다 10을 곱한다.
  }
  alert(testFn(99));
})
...
arrows[2].addEventListener("click", () => {
  // 함수 정의 부분이 한 줄 --> {return} 생략 가능
  const minusFn = (a, b) => a - b;
  alert(minusFn(10, 3)); // 7이 나온다

  // 매개 변수 1개, 정의 내용 return 한 줄
  const dobleFn = num => num * 2;
...
/* STACK 구조  
  ...
  - 한 쪽으로만 들어가고
    한 쪽으로만 나오는 형태 ex) 컵의 구조

  - 메모리에서 실행된 함수들을
    리스트업할 때 사용된다 ( 함수 호출 구조는 stack 형식 )
